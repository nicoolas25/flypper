...
from flypper.entities.flag import Flag, FlagData

def test_deleted_flag():
    flag = create_flag(deleted=True)
    assert not flag.is_enabled(entries={})
...
def test_enabled_flag():
    ...
    assert flag.is_enabled(entries={})
...
def test_enabled_for_actors_flag():
    ...
    assert flag.is_enabled(entries={"user_id": "6"})
    assert not flag.is_enabled(entries={"user_id": "5"})
    assert not flag.is_enabled(entries={})
...
def test_enabled_for_percentage_of_actors_flag():
    ...
    assert flag.is_enabled(entries={"user_id": "7"})
    assert not flag.is_enabled(entries={"user_id": "53"})
    assert not flag.is_enabled(entries={})
